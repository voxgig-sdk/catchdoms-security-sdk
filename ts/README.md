# CatchdomsSecurity TypeScript SDK



The TypeScript SDK for the CatchdomsSecurity API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Domain()` — each with a small set of operations (`list`)
...
```ts
const client = new CatchdomsSecuritySDK({ apikey: '...' })
...
```ts
new CatchdomsSecuritySDK(options?: {
  apikey?: string
  base?: string
  prefix?: string
  suffix?: string
...
| `tester(testopts?, sdkopts?)` | `CatchdomsSecuritySDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `CatchdomsSecuritySDK.test(testopts?, sdkopts?)` | `CatchdomsSecuritySDK` | Create a test-mode client. |
...
| `client` | `client(): CatchdomsSecuritySDK` | Return the parent SDK client. |
...
Create an instance: `const pending_delete = client.PendingDelete()`
...
```ts
const pending_deletes = await client.PendingDelete().list()
```
...
catchdoms-security/
├── src/
│   ├── CatchdomsSecuritySDK.ts        # Main SDK class
