...
describe('PendingDeleteEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'pending_delete.' + op, live)) return
    }
    ...
    const pending_delete_ref01_list = await pending_delete_ref01_ent.list(pending_delete_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['CATCHDOMS_SECURITY_TEST_PENDING_DELETE_ENTID']
