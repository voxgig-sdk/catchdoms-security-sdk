...
const __1 = require("..");
const SDK_NAME = 'CatchdomsSecuritySDK';
...
const TEST_SEED = { "entity": { "domain": { "test01": { "id": "test01" } }, "mcp": { "test01": { "id": "test01" } }, "pending_delete": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+CatchdomsSecuritySDK\b/.test(code) ||
        /\bCatchdomsSecuritySDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+CatchdomsSecuritySDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/CatchdomsSecuritySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
