...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("CATCHDOMSSECURITY_TEST_LIVE") == "TRUE" ||
		os.Getenv("CATCHDOMSSECURITY_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("CATCHDOMSSECURITY_TEST_EXPLAIN"); explain != "" {
