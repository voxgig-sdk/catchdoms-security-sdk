...
func TestMcpEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Mcp(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Mcp(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CATCHDOMSSECURITY_TEST_MCP_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		mcpRef01Match := map[string]any{}

		mcpRef01ListResult, err := mcpRef01Ent.List(mcpRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, mcpRef01ListOk := mcpRef01ListResult.([]any)
		if !mcpRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", mcpRef01ListResult)
		}
...
func mcpBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CATCHDOMSSECURITY_TEST_MCP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CATCHDOMSSECURITY_TEST_MCP_ENTID"])
