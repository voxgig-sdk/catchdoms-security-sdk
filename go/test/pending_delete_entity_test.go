...
func TestPendingDeleteEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CATCHDOMSSECURITY_TEST_PENDING_DELETE_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		pendingDeleteRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.pending_delete", setup.data)))
...
func pending_deleteBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CATCHDOMSSECURITY_TEST_PENDING_DELETE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CATCHDOMSSECURITY_TEST_PENDING_DELETE_ENTID"])
