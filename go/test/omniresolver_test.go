...
import (
	...
	omni "github.com/voxgig-sdk/catchdoms-security-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CatchdomsSecurityError); is {
				out := map[string]any{
					"name":    "CatchdomsSecurityError",
...
func omniCtx(arg any, client *sdk.CatchdomsSecuritySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CatchdomsSecuritySDK); is {
			client = live
