// Command CatchdomsSecurity-cli is a CLI + REPL for the CatchdomsSecurity SDK,
...
const prompt = "catchdoms-security"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "domain mcp pending_delete"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CATCHDOMS_SECURITY_APIKEY (api key), CATCHDOMS_SECURITY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CATCHDOMS_SECURITY_APIKEY carries the API key and
	// CATCHDOMS_SECURITY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CATCHDOMS_SECURITY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CATCHDOMS_SECURITY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCatchdomsSecuritySDK(opts)
