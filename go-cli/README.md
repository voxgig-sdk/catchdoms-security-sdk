# catchdoms-security-cli

boru-driven command-line client **and** interactive REPL for the CatchdomsSecurity
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/catchdoms-security-cli)
make build

# 2. See usage (words, entities, env vars)
./catchdoms-security-cli --help

# 3. Provide credentials once, via the environment
export CATCHDOMS_SECURITY_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./catchdoms-security-cli list domain
./catchdoms-security-cli list mcp

# 5. Override the API base URL for a single call
CATCHDOMS_SECURITY_BASE=https://api.example.com ./catchdoms-security-cli list domain

# 6. No arguments -> interactive REPL
./catchdoms-security-cli
catchdoms-security> list domain
catchdoms-security> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/catchdoms-security-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/catchdoms-security-cli list domain
   ```
...
Run with no arguments to open a REPL (prompt `catchdoms-security>`). Each line is
evaluated as its own boru expression:

```text
$ ./catchdoms-security-cli
catchdoms-security> list domain
catchdoms-security> /help
catchdoms-security> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `domain`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/catchdoms-security-cli`. |
...
(`./catchdoms-security-cli <expr>`) and interactively (the REPL), and expressions compose the
