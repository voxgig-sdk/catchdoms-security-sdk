...
require("catchdoms-security_types")
...
local CatchdomsSecuritySDK = {}
CatchdomsSecuritySDK.__index = CatchdomsSecuritySDK
...
CatchdomsSecuritySDK._make_feature = _make_feature


function CatchdomsSecuritySDK.new(options)
  local self = setmetatable({}, CatchdomsSecuritySDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function CatchdomsSecuritySDK:options_map()
...
function CatchdomsSecuritySDK:get_utility()
  return Utility.copy(self._utility)
end


function CatchdomsSecuritySDK:get_root_ctx()
  return self._rootctx
end


function CatchdomsSecuritySDK:prepare(fetchargs)
...
function CatchdomsSecuritySDK:direct(fetchargs)
...
function CatchdomsSecuritySDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function CatchdomsSecuritySDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "CatchdomsSecuritySDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function CatchdomsSecuritySDK:_raw_request(fetchargs)
  local utility = self._utility
...
function CatchdomsSecuritySDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "CatchdomsSecuritySDK: graphql: " .. msg
    res.graphql = errors
  end
...
function CatchdomsSecuritySDK:Domain(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function CatchdomsSecuritySDK:Mcp(data)
...
-- Idiomatic facade: client:PendingDelete():list() / client:PendingDelete():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function CatchdomsSecuritySDK:PendingDelete(data)
  local EntityMod = require("entity.pending_delete_entity")
  if data == nil then
    if self._pending_delete == nil then
      self._pending_delete = EntityMod.new(self, nil)
    end
...
function CatchdomsSecuritySDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = CatchdomsSecuritySDK.new(sdkopts)
  sdk.mode = "test"
