...
local PendingDeleteEntity = {}
PendingDeleteEntity.__index = PendingDeleteEntity


function PendingDeleteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PendingDeleteEntity)
  self._name = "pending_delete"
  self._client = client
...
function PendingDeleteEntity:get_name()
  return self._name
end


function PendingDeleteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PendingDeleteEntity.new(self._client, opts)
end
...
function PendingDeleteEntity:mark_deleted()
  self._deleted = true
end


function PendingDeleteEntity:deleted()
  return true == self._deleted
end


function PendingDeleteEntity:data_set(args)
  if args ~= nil then
...
function PendingDeleteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PendingDeleteEntity:match_set(args)
  if args ~= nil then
...
function PendingDeleteEntity:match_get()
...
function PendingDeleteEntity:stream(action, args, callopts)
...
---@param reqmatch PendingDeleteListMatch
---@param ctrl? table
---@return PendingDelete[]
---@return string? err
function PendingDeleteEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function PendingDeleteEntity:_run_op(ctx, post_done)
  local utility = self._utility
