...
local McpEntity = {}
McpEntity.__index = McpEntity


function McpEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, McpEntity)
  self._name = "mcp"
  self._client = client
...
function McpEntity:get_name()
  return self._name
end


function McpEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return McpEntity.new(self._client, opts)
end
...
function McpEntity:mark_deleted()
  self._deleted = true
end


function McpEntity:deleted()
  return true == self._deleted
end


function McpEntity:data_set(args)
  if args ~= nil then
...
function McpEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function McpEntity:match_set(args)
  if args ~= nil then
...
function McpEntity:stream(action, args, callopts)
...
---@param reqmatch McpListMatch
---@param ctrl? table
---@return Mcp[]
---@return string? err
function McpEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function McpEntity:_run_op(ctx, post_done)
  local utility = self._utility
