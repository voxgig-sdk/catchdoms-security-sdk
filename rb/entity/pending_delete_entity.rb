...
class PendingDeleteEntity
  ...
  def make
    opts = @_entopts.dup
    PendingDeleteEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CatchdomsSecurityHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [PendingDelete, Hash] the current PendingDelete data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CatchdomsSecurityHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of PendingDelete fields)
  def match_get
  ...
  #
  # @param reqmatch [PendingDeleteListMatch, Hash, nil] match filter (any subset of
  #   PendingDelete fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<PendingDelete>, Array] the matching PendingDelete items; raises CatchdomsSecurityError on failure
