...
class McpEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Mcp(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Mcp(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CATCHDOMSSECURITY_TEST_MCP_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    mcp_ref01_data = nil
    if mcp_ref01_data_raw.length > 0
    ...
    mcp_ref01_list_result = mcp_ref01_ent.list(mcp_ref01_match, nil)
    assert mcp_ref01_list_result.is_a?(Array)
...
def mcp_basic_setup(extra)
  ...
  entid_env_raw = ENV["CATCHDOMSSECURITY_TEST_MCP_ENTID"]
