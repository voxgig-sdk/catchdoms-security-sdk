...
class PendingDeleteDirectTest < Minitest::Test
  def test_direct_list_pending_delete
    setup = pending_delete_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-pending_delete", setup[:live] ? "live" : "unit")
    if _should_skip
...
def pending_delete_direct_setup(mockres)
