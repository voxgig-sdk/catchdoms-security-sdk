...
require_relative 'CatchdomsSecurity_types'
...
class CatchdomsSecuritySDK
  ...
  def initialize(options = {})
    ...
    feature_opts = CatchdomsSecurityHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CatchdomsSecurityHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, CatchdomsSecurityFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    CatchdomsSecurityUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = CatchdomsSecurityHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = CatchdomsSecurityHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = CatchdomsSecurityHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
    ...
    ctx.spec = CatchdomsSecuritySpec.new({
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => CatchdomsSecurityError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue CatchdomsSecurityError => err
    ...
    if fetched.is_a?(Hash)
      status = CatchdomsSecurityHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CatchdomsSecurityError.new(
        "graphql_error", "CatchdomsSecuritySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.PendingDelete.list / client.PendingDelete.load({ "id" => ... })
  def PendingDelete(data = nil)
    require_relative 'entity/pending_delete_entity'
    PendingDeleteEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = CatchdomsSecuritySDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
