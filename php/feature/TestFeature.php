...
// CatchdomsSecurity SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class CatchdomsSecurityTestFeature extends CatchdomsSecurityBaseFeature
{
    ...
    public function init(CatchdomsSecurityContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (CatchdomsSecurityContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = CatchdomsSecurityParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (CatchdomsSecurityContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(CatchdomsSecurityContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = CatchdomsSecurityParam::call($ctx, $k);
