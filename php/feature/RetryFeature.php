...
// CatchdomsSecurity SDK retry feature
...
class CatchdomsSecurityRetryFeature extends CatchdomsSecurityBaseFeature
{
    ...
    public function init(CatchdomsSecurityContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CatchdomsSecurityContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CatchdomsSecurityContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CatchdomsSecurityContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
