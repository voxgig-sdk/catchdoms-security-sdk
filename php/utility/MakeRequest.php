...
class CatchdomsSecurityMakeRequest
{
    public static function call(CatchdomsSecurityContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new CatchdomsSecurityResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new CatchdomsSecurityResponse($fetched);
        } else {
