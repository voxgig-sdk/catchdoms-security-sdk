...
class CatchdomsSecurityMakeError
{
    public static function call(?CatchdomsSecurityContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CatchdomsSecurityContext([], null);
        }
        $op = $ctx->op ?? new CatchdomsSecurityOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CatchdomsSecurityResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CatchdomsSecurityError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new CatchdomsSecurityError('', $msg, $ctx);
        ...
        if ($err instanceof CatchdomsSecurityError) {
