...
class CatchdomsSecuritySDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = CatchdomsSecurityHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CatchdomsSecurityHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CatchdomsSecurityFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CatchdomsSecurityFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CatchdomsSecurityUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CatchdomsSecurityHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CatchdomsSecurityHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CatchdomsSecurityHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CatchdomsSecurityError($op . "_allow",
                "CatchdomsSecuritySDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CatchdomsSecurityHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CatchdomsSecurityError("graphql_error",
                "CatchdomsSecuritySDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_pending_delete = null;

    // Canonical facade: $client->PendingDelete()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->pending_delete()
    // resolves here too.
    public function PendingDelete($data = null)
    {
        require_once __DIR__ . '/entity/pending_delete_entity.php';
        if ($data === null) {
            if ($this->_pending_delete === null) {
                $this->_pending_delete = new PendingDeleteEntity($this, null);
            }
            return $this->_pending_delete;
        }
        return new PendingDeleteEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new CatchdomsSecuritySDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
