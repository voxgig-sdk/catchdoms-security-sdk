...
// CatchdomsSecurity SDK PendingDelete entity
...
class PendingDeleteEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PendingDeleteEntity($this->_client, $opts);
    }

    /**
     * @param PendingDelete|array $args PendingDelete data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CatchdomsSecurityHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return PendingDelete|array The current PendingDelete data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of PendingDelete fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CatchdomsSecurityHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of PendingDelete fields).
     */
    ...
    /**
     * List PendingDelete items matching the given filter.
     *
     * @param PendingDeleteListMatch|array|null $reqmatch Match filter (any subset
     *   of PendingDelete fields) as an assoc-array; PendingDeleteListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return PendingDelete[]|array A list of PendingDelete items as assoc-arrays at
     *   the SDK boundary; throws CatchdomsSecurityError on failure (item-5 convention).
     */
