...
//    only - a typed CatchdomsSecurityContext there would make every ctx
...
final class CatchdomsSecurityOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new CatchdomsSecurityContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new CatchdomsSecuritySpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new CatchdomsSecurityResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new CatchdomsSecurityError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new CatchdomsSecurityResponse($clean['response']);
    ...
    public static function err_from_map($m): ?CatchdomsSecurityError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CatchdomsSecurityError(is_string($code) ? $code : '', $msg);
    }
