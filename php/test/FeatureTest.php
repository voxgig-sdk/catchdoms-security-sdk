...
// CatchdomsSecurity SDK feature test
//
...
class FtCtrl extends CatchdomsSecurityControl
{
...
class FtClient
{
    ...
    private CatchdomsSecurityUtility $utility;

    public function __construct(CatchdomsSecurityUtility $utility, array $options)
    {
    ...
    public function get_utility(): CatchdomsSecurityUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CatchdomsSecurityContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CatchdomsSecurityContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CatchdomsSecurityContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CatchdomsSecurityConfig::make_feature($name);
    ...
    private static function buildUrl(CatchdomsSecuritySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CatchdomsSecurityError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CatchdomsSecurityResponse($fetched);
        }
    ...
    private function fail(CatchdomsSecurityContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CatchdomsSecurityContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
