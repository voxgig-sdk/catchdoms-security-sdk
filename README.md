...
Catchdoms Security API client, generated from the OpenAPI spec.

This API provides security verification to protect websites from malicious bots.

Learn more about Catchdoms Security API at [catchdoms.com](https://catchdoms.com).

This is an unofficial SDK for the Catchdoms Security public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Domain, Mcp and PendingDelete — that you
...
```bash
cd go-mcp && go build -o catchdoms-security-mcp .
```
...
```json
{
  "mcpServers": {
    "catchdoms-security": {
      "command": "/abs/path/to/catchdoms-security-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **Domain** | The Domain entity (list). | `/api/domains` |
| **Mcp** | The Mcp entity (list). | `/mcp/catchdoms` |
| **PendingDelete** | The PendingDelete entity (list). | `/api/pending-delete` |
...
```go
import sdk "github.com/voxgig-sdk/catchdoms-security-sdk/go"
...
Generated from the Catchdoms Security API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
