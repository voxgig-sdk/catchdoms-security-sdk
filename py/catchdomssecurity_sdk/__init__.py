...
from catchdomssecurity_sdk.utility import register

# Load features
from catchdomssecurity_sdk.feature.base_feature import CatchdomsSecurityBaseFeature
from catchdomssecurity_sdk.features import _has_feature, _make_feature
...
class CatchdomsSecuritySDK:
    ...
    def __init__(self, options=None):
        ...
        utility = CatchdomsSecurityUtility()
    ...
    def get_utility(self):
        return CatchdomsSecurityUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("CatchdomsSecuritySDK: graphql: " + str(msg))
            res["graphql"] = errors
    ...
    def Domain(self, data=None) -> "DomainEntity":
        ...
        from catchdomssecurity_sdk.entity.domain_entity import DomainEntity
        return DomainEntity(self, data)


    def Mcp(self, data=None) -> "McpEntity":
        """Entity factory: client.Mcp().list() / client.Mcp().load({"id": ...})."""
        from catchdomssecurity_sdk.entity.mcp_entity import McpEntity
        return McpEntity(self, data)


    def PendingDelete(self, data=None) -> "PendingDeleteEntity":
        """Entity factory: client.PendingDelete().list() / client.PendingDelete().load({"id": ...})."""
        from catchdomssecurity_sdk.entity.pending_delete_entity import PendingDeleteEntity
        return PendingDeleteEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "CatchdomsSecuritySDK":
        if sdkopts is None:
            sdkopts = {}
