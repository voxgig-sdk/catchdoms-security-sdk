...
class TestPendingDeleteDirect:

    def test_should_direct_list_pending_delete(self):
        setup = _pending_delete_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-pending_delete", "live" if setup["live"] else "unit")
        if _skip:
...
def _pending_delete_direct_setup(mockres):
