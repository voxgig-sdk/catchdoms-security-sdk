# CatchdomsSecurity SDK test runner
...
class CatchdomsSecurityTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CatchdomsSecurityTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CatchdomsSecurityTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CatchdomsSecurityTestRunner.getenv("CATCHDOMS_SECURITY_TEST_LIVE")
        override = CatchdomsSecurityTestRunner.getenv("CATCHDOMS_SECURITY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CatchdomsSecurityTestRunner.getenv(key)
        ...
        explain = CatchdomsSecurityTestRunner.getenv("CATCHDOMS_SECURITY_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if CatchdomsSecurityTestRunner._test_control is not None:
            return CatchdomsSecurityTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CatchdomsSecurityTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CatchdomsSecurityTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CatchdomsSecurityTestRunner.load_test_control()
...
def load_env_local():
    CatchdomsSecurityTestRunner.load_env_local()


def env_override(m):
    return CatchdomsSecurityTestRunner.env_override(m)


def entity_data(v):
    return CatchdomsSecurityTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CatchdomsSecurityTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CatchdomsSecurityTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CatchdomsSecurityTestRunner.load_test_control()


def live_client_options():
    return CatchdomsSecurityTestRunner.live_client_options()


def live_delay_ms():
    return CatchdomsSecurityTestRunner.live_delay_ms()
