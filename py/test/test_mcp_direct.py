...
class TestMcpDirect:
    ...
    def test_should_direct_list_mcp(self):
        setup = _mcp_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-mcp", "live" if setup["live"] else "unit")
        if _skip:
