...
module CatchdomsSecurityUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CatchdomsSecurityResponse.new({})
    ...
    if fetch_err
      ...
      response = CatchdomsSecurityResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CatchdomsSecurityResponse.new(fetched)
    else
