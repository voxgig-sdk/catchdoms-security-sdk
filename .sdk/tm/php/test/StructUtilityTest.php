...
// CatchdomsSecurity SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = CatchdomsSecurityOmni::makeStructRunner(
    ...
    {
        return CatchdomsSecurityOmni::objmodel(CatchdomsSecurityOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return CatchdomsSecurityOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return CatchdomsSecurityOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(CatchdomsSecurityOmni::structfix($now, Struct::undef()))
                        !== json_encode(CatchdomsSecurityOmni::structfix($val, Struct::undef()))) {
